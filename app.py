...
def require_session_defaults() -> None:
    ...
    st.session_state.setdefault("admin_usage_example", "")
    st.session_state.setdefault("append_notice", "")
    st.session_state.setdefault("ai_cached_word", "")
    st.session_state.setdefault("gemini_model_name", None)


@st.cache_data(ttl=120, show_spinner=False)
def load_student_names_for_select() -> tuple[str, ...]:
    """Unique student names from column A (cached to limit Sheet reads)."""
    try:
        cfg = st.secrets["connections"]["gsheets"]
        client = gspread.service_account_from_dict(dict(cfg["service_account"]))
        ws = client.open_by_url(cfg["spreadsheet"]).get_worksheet(0)
...
def resolve_canonical_student_name(entered: str) -> str:
    """Return sheet spelling for this student if found; matching is case-insensitive."""
    df = st.session_state.get("sheet_df")
    if df is None or "Student Name" not in getattr(df, "columns", []):
        return entered.strip()
    target = entered.strip().casefold()
    if not target:
        return entered.strip()
    for raw in df["Student Name"].dropna().astype(str).unique():
        s = str(raw).strip()
        if s.casefold() == target:
            return s
    return entered.strip()
...
require_session_defaults()
student_name_options = list(load_student_names_for_select())
if student_name_options:
    preferred = st.session_state.get("active_student_name") or st.session_state.get(
        "student_name_select"
    )
    if preferred and "student_name_select" not in st.session_state:
        for n in student_name_options:
            if n.casefold() == preferred.casefold():
                st.session_state["student_name_select"] = n
                break
    student_name = st.selectbox(
        "Student Name",
        options=student_name_options,
        key="student_name_select",
        help="Choose your name. Matching is not affected by capital letters.",
    )
else:
    student_name = st.text_input(
        "Student Name",
        placeholder="Type your name (uppercase/lowercase OK)",
        key="student_name_input",
    ).strip()
search_clicked = st.button("Search", use_container_width=True, key="student_search_button")
...
def extract_wait_seconds(error_text: str) -> int | None:
    patterns = [
        r"seconds:\s*(\d+)",
        r"retry in\s*(\d+)\s*s",
...
def word_exists_for_student(student: str, word: str) -> bool:
    """Return True if this word already exists for this student in cached sheet data."""
    df = st.session_state.get("sheet_df")
    if df is None or "Student Name" not in df.columns or "Word" not in df.columns:
        return False
    s_norm = student.strip().casefold()
    w_norm = word.strip().casefold()
    if not w_norm:
        return False
    mask = (
        df["Student Name"].astype(str).str.strip().str.casefold() == s_norm
    ) & (df["Word"].astype(str).str.strip().str.casefold() == w_norm)
    return bool(mask.any())


def _resolve_gemini_model_name(api_key: str, preferred_model: str | None) -> str:
    cached = st.session_state.get("gemini_model_name")
    if cached:
        return cached
...
def generate_vocab_with_ai(word: str) -> tuple[str, str]:
    """
    Call Gemini generateContent only from explicit user actions.
    Caches resolved model name in session_state to avoid list_models on every call.
    """
    ai_cfg = st.secrets.get("ai", {})
    api_key = ai_cfg.get("gemini_api_key") or ai_cfg.get("api_key") or os.environ.get(
        "GEMINI_API_KEY"
    ...
    genai.configure(api_key=api_key)
    model_name = _resolve_gemini_model_name(api_key, preferred_model)
...
def _clear_teacher_form_after_append() -> None:
    """Reset inputs and AI-related session state after a successful append."""
    keys_to_drop = [
        "admin_student_input",
        "admin_lesson_input",
        "admin_word_input",
        "admin_student_save_input",
        "admin_lesson_save_input",
        "admin_word_save_input",
        "admin_translation_save_input",
        "admin_example_save_input",
    ]
    for k in keys_to_drop:
        st.session_state.pop(k, None)
    st.session_state["admin_translation"] = ""
    st.session_state["admin_usage_example"] = ""
    st.session_state["ai_cached_word"] = ""
    st.session_state["admin_lesson_input"] = "1"
    st.session_state["admin_lesson_save_input"] = "1"


def render_admin_panel() -> None:
    st.sidebar.markdown("## Teacher Terminal")
    admin_password = st.secrets.get("admin", {}).get("password")
    if not admin_password:
        st.sidebar.info("Set [admin].password in secrets.toml to enable admin mode.")
        return
    ...
    if df is not None and "Student Name" in df.columns:
        student_names = sorted(
            {
                str(x).strip()
                for x in df["Student Name"].dropna().astype(str).tolist()
                if str(x).strip()
            },
            key=lambda x: x.casefold(),
        )
    ...
    st.sidebar.markdown("---")
    if student_names:
        admin_student = st.sidebar.selectbox(
            "Student Name",
            options=student_names,
            key="admin_student_input",
        )
    else:
        admin_student = st.sidebar.text_input("Student Name", key="admin_student_input")
    admin_lesson = st.sidebar.text_input("Lesson", value="1", key="admin_lesson_input")
    admin_word = st.sidebar.text_input("New Word", key="admin_word_input")

    quota_until = float(st.session_state.get("gemini_quota_until", 0))
    now_ts = time.time()
    in_quota_cooldown = now_ts < quota_until
    if quota_until > 0 and not in_quota_cooldown:
        st.session_state["gemini_quota_until"] = 0.0

    # Gemini is ONLY invoked inside this explicit button handler (not on reruns).
    if st.sidebar.button(
        "Generate Translation + Example with AI",
        use_container_width=True,
        key="admin_generate_ai_button",
        disabled=in_quota_cooldown,
    ):
        word_clean = (st.session_state.get("admin_word_input") or "").strip()
        student_clean = (st.session_state.get("admin_student_input") or "").strip()

        if not word_clean:
            st.sidebar.warning("Please enter a word first.")
        elif not student_clean:
            st.sidebar.warning("Please enter a student name first.")
        else:
            cache_key = word_clean.casefold()
            if (
                st.session_state.get("ai_cached_word") == cache_key
                and st.session_state.get("admin_translation")
                and st.session_state.get("admin_usage_example")
            ):
                st.sidebar.info("Using saved AI result from this session (no API call).")
            elif word_exists_for_student(student_clean, word_clean):
                st.sidebar.warning("Word already exists in your list.")
            else:
                try:
                    with st.spinner("Calling Gemini…"):
                        translation, usage_example = generate_vocab_with_ai(word_clean)
                    st.session_state["admin_translation"] = translation
                    st.session_state["admin_usage_example"] = usage_example
                    st.session_state["ai_cached_word"] = cache_key
                    st.session_state["admin_word_save_input"] = word_clean
                    st.session_state["admin_translation_save_input"] = translation
                    st.session_state["admin_example_save_input"] = usage_example
                    st.sidebar.success("AI content generated. Review and save below.")
                except ResourceExhausted as exc:
                    err = format_error(exc)
                    wait_s = extract_wait_seconds(err) or 60
                    st.session_state["gemini_quota_until"] = time.time() + wait_s
                    st.sidebar.warning(
                        f"AI quota reached. Please wait **{wait_s}** seconds before generating again."
                    )
                    if wait_s > 0:
                        st_autorefresh(interval=1000, key="teacher_quota_tick")
                except Exception as exc:
                    st.sidebar.error(f"AI generation failed: {format_error(exc)}")

    if in_quota_cooldown:
        left = max(0, int(quota_until - now_ts))
        st.sidebar.warning(f"Quota cooldown: **{left}** s remaining.")
        st_autorefresh(interval=1000, key="teacher_quota_tick")

    # Keep save form fields in sync with session AI cache (no API calls on reruns).
    if (
        "admin_translation_save_input" not in st.session_state
        and st.session_state.get("admin_translation")
    ):
        st.session_state["admin_translation_save_input"] = st.session_state["admin_translation"]
    if (
        "admin_example_save_input" not in st.session_state
        and st.session_state.get("admin_usage_example")
    ):
        st.session_state["admin_example_save_input"] = st.session_state["admin_usage_example"]

    if "admin_lesson_save_input" not in st.session_state:
        st.session_state["admin_lesson_save_input"] = st.session_state.get("admin_lesson_input", "1") or "1"
    if "admin_student_save_input" not in st.session_state:
        st.session_state["admin_student_save_input"] = st.session_state.get("admin_student_input", "") or ""
    if "admin_word_save_input" not in st.session_state:
        st.session_state["admin_word_save_input"] = st.session_state.get("admin_word_input", "") or ""
    ...
    if save_clicked:
        fs = (st.session_state.get("admin_student_save_input") or "").strip()
        fl = (st.session_state.get("admin_lesson_save_input") or "").strip()
        fw = (st.session_state.get("admin_word_save_input") or "").strip()
        ft = (st.session_state.get("admin_translation_save_input") or "").strip()
        fe = (st.session_state.get("admin_example_save_input") or "").strip()

        if not fs or not fl or not fw:
            st.sidebar.warning("Student Name, Lesson, and Word are required.")
            return
        if not ft or not fe:
            st.sidebar.warning("Generate or enter Translation/Definition and Usage Example.")
        ...
        try:
            ...
            refresh_data(force=True)
            st.session_state["active_student_name"] = fs
            st.session_state["lesson_filter"] = "All"
            st.session_state["quick_search"] = ""
            st.session_state["word_filter"] = ""
            st.session_state["translation_filter"] = ""
            ...
            st.session_state["page_number"] = 1
            st.session_state["append_notice"] = "New word added and table refreshed."
            _clear_teacher_form_after_append()
            st.sidebar.success("New row added. Form cleared for next entry.")
            st.rerun()
        except Exception as exc:
            st.sidebar.error(f"Append failed: {format_error(exc)}")
...
if search_clicked:
    if not student_name:
        st.warning("Please choose or enter a student name.")
        st.stop()
    try:
        with st.spinner("Loading words..."):
            refresh_data(force=True)
        ...
        st.error(f"Error loading data: {format_error(exc)}")
        st.stop()
    st.session_state["active_student_name"] = resolve_canonical_student_name(student_name)
